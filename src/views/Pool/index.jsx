...
import { Box, OutlinedInput, InputAdornment, Checkbox } from "@material-ui/core";
import styled from "styled-components";
import { ethers } from "ethers";
import { useWeb3Context } from '../../hooks/web3Context';
import { DOX_LOCK, DOX_UNLOCK } from '../../abi/address.js'
...
const unlockcompound = [40.14 / 33.76, 40.01 / 33.76, 39.85 / 33.76, 39.51 / 33.76];
const lockcompound = [
  [46.24 / 38.03, 46.07 / 38.03, 45.87 / 38.03, 45.42 / 38.03],
  [87.34 / 62.83, 86.74 / 62.83, 86.05 / 62.83, 84.53 / 62.83]
]
...
function Pool({ account, unlockinfo, unlockallow, lockinfo, lockallow, lockups }) {
  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      if (modalopen % 10 === 1) {
        const unLockContract = new ethers.Contract(DOX_UNLOCK, UnLockABI, signer);
        if (modalopen === 1) {
          await unLockContract.deposit(ethers.utils.parseEther(temp), { value: unlockinfo.performanceFee });
        }
        else
          await unLockContract.withdraw(ethers.utils.parseEther(temp), { value: unlockinfo.performanceFee });
      }
      else {
        const LockContract = new ethers.Contract(DOX_LOCK, LockABI, signer)
        if (modalopen < 10) {
          await LockContract.deposit(ethers.utils.parseEther(temp), modalopen % 10 - 2, { value: lockinfo.performanceFee });
        }
        else
          await LockContract.withdraw(ethers.utils.parseEther(temp), modalopen % 10 - 2, { value: lockinfo.performanceFee });
      }
  ...
  const onCompoundReward = async (i) => {
    ...
    try {
      const signer = provider.getSigner();
      if (i === 1) {
        const contract = new ethers.Contract(DOX_UNLOCK, UnLockABI, signer);
        await contract.compoundReward({ value: unlockinfo.performanceFee });
      }
      else {
        const contract = new ethers.Contract(DOX_LOCK, LockABI, signer)
        await contract.compoundReward(i - 2, { value: lockinfo.performanceFee });
      }
    }
    catch (error) {
      console.log(error);
  ...
  const onCompoundReflection = async (i) => {
    ...
    try {
      ...
      if (i === 1) {
        const contract = new ethers.Contract(DOX_UNLOCK, UnLockABI, signer);
        await contract.compoundDividend({ value: unlockinfo.performanceFee });
      }
      else {
        const contract = new ethers.Contract(DOX_LOCK, LockABI, signer)
        await contract.compoundDividend(i - 2, { value: lockinfo.performanceFee });
      }
    }
    catch (error) {
      console.log(error);
  ...
  const onHarvestReward = async (i) => {
    ...
    try {
      ...
      if (i === 1) {
        const contract = new ethers.Contract(DOX_UNLOCK, UnLockABI, signer);
        await contract.claimReward({ value: unlockinfo.performanceFee });
      }
      else {
        const contract = new ethers.Contract(DOX_LOCK, LockABI, signer)
        await contract.claimReward(i - 2, { value: lockinfo.performanceFee });
      }
    }
    catch (error) {
      console.log(error);
  ...
  const onHarvestReflection = async (i) => {
    ...
    try {
      ...
      if (i === 1) {
        const contract = new ethers.Contract(DOX_UNLOCK, UnLockABI, signer);
        await contract.claimDividend({ value: unlockinfo.performanceFee });
      }
      else {
        const contract = new ethers.Contract(DOX_LOCK, LockABI, signer)
        await contract.claimDividend(i - 2, { value: lockinfo.performanceFee });
      }
    }
    catch (error) {
      console.log(error);
  ...
  const onApproveContract = async (type, address) => {
    ...
    try {
      const signer = provider.getSigner();
      const tokenContract = new ethers.Contract(address, DoxTokenABI, signer);
      await tokenContract.approve(type === 1 ? DOX_UNLOCK : DOX_LOCK, "115792089237316195423570985008687907853269984665640564039457584007913129639935");
      // setPending(false);
    }
    catch (error) {
      console.log(error);
  ...
  const CalculateRate = (type) => {
    if (type === 1) {
      if (compoundcalc)
        return Number(stakeday * unlockinfo?.rate * unlockcompound[compoundday] / 36500);
      return Number(stakeday * unlockinfo?.rate / 36500);
    }
    else if (type > 1) {
      console.log(type - 2);
      if (compoundcalc)
        return Number(stakeday * lockups[type - 2]?.rate * lockcompound[type - 2][compoundday] / 36500);
  ...
  return (
    <StyledContainer>
        ...
        <Box display={'flex'} justifyContent={'space-between'} fontSize={'18px'} mb={'5px'}>
          <Box>
            = {calcshowtype ? numberWithCommas((Number(calcamount) / unlockinfo?.stakingTokenInfo.price).toFixed(2))
              : numberWithCommas(Number(unlockinfo?.stakingTokenInfo.price * Number(calcamount)).toFixed(2))} {calcshowtype ? 'ETHDOX' : 'USD'}
          </Box>
          <Box>
          ...
          endAdornment={
            <InputAdornment position="start">
              <Box display={'flex'} alignItems={'center'}>
                <Box fontSize={'14px'} mr={'5px'}>{calcshowtype ? 'USD' : 'ETHDOX '}</Box>
        ...
        <Box borderRadius={'16px'} mt={'40px'} border={'1px solid black'} padding={'30px'} mx={'20px'} mb={'20px'}>
          ...
          <Box mt={'10px'} fontWeight={'bold'}> ~ {
            calcshowtype ? (Number(calcamount) * CalculateRate(calcmodal) / unlockinfo?.stakingTokenInfo.price).toFixed(3) :
              (Number(calcamount) * CalculateRate(calcmodal)).toFixed(3)} ETHDOX({(CalculateRate(calcmodal) * 100).toFixed(2)}%)</Box>
        </Box>
      ...
      <LockPanel>
        ...
        {lockups && lockups.map((data, i) => <Paper width={'550px'} height={'fit-content'}>
          ...
          <Box display={'flex'} justifyContent={'space-between'} alignItems={'center'} >
            ...
            <Box >
              <Box width={'60px'} height={'60px'} bgcolor={'white'} borderRadius={'50%'}>
                <img src={'/images/dox.png'} width={'100%'} height={'100%'} alt={'bnb'} />
              </Box>
              <Box position={'relative'} mt={'-50px'} ml={'10px'} width={'80px'} height={'80px'} bgcolor={'white'} borderRadius={'50%'}>
                <img src={'/images/dox.png'} width={'100%'} height={'100%'} alt={'dox'} />
              </Box>
            </Box>
          </Box>
          <DataField>
            ...
            <Box display={'flex'} justifyContent={'space-between'} alignItems={'center'} fontSize={'24px'}>
              <Box >
                ...
                {data.lockup ?
                  <Box fontSize={'16px'}>
                    {numberWithCommas(Number(data?.pendingReflection * lockinfo?.reflectionTokenInfo.ethPrice).toFixed(2))} USD
                  </Box> :
                  <Skeleton variant={'text'} width={'60px'} height={'100%'} style={{ transform: 'unset' }} />
                }
              </Box>
          ...
          {showlockdetail === i ? <Box mt={'20px'} fontSize={'18px'}>
            ...
            <Box display={'flex'} justifyContent={'end'}>
              <a href={'https://pancakeswap.finance/info/token/0xaFb64E73dEf6fAa8B6Ef9a6fb7312d5C4C15ebDB'} target={'_blank'}>
                See Token Info
              </a>
            </Box>
          </Box> : ''
        ...
        <Paper width={'550px'} height={'fit-content'} >
            ...
            <Box display={'flex'} justifyContent={'space-between'} alignItems={'center'} fontSize={'24px'}>
              <Box>
                ...
                }
                {
                  unlockinfo ?
                    <Box fontSize={'16px'}>
                      {numberWithCommas((unlockinfo?.pendingReflection * unlockinfo?.reflectionTokenInfo.ethPrice).toFixed(2))} USD
                    </Box> :
                    <Box mt={'5px'}>
                      <Skeleton variant={'text'} width={'60px'} height={'100%'} style={{ transform: 'unset' }} />
                    </Box>
              ...
              <Box fontSize={'24px'} display={'flex'} justifyContent={'space-between'} alignItems={'center'}>
                <Box mt={'10px'}>
                  ...
                  }
                  {
                    unlockinfo.userinfo ?
                      <Box fontSize={'16px'}>
                        {numberWithCommas(Number(unlockinfo?.userinfo?.amount * lockinfo?.stakingTokenInfo.price).toFixed(2))} USD
                      </Box> :
                      <Box mt={'5px'}>
                        <Skeleton variant={'text'} width={'60px'} height={'100%'} style={{ transform: 'unset' }} />
                      </Box>
                ...
                <Box>
                  {
                    !unlockinfo?.userinfo?.amount ?
                      <StyledButton onClick={() => { setModalOpen(1); setModalBalance(lockinfo?.stakingTokenInfo.balance) }} disabled={pending}>
                        Stake
                      </StyledButton> : ''
                  }
                  {
                    unlockinfo?.userinfo?.amount ?
                      <Action>
                        <StyledButton style={{ width: 'fit-content', padding: '0px 25px', fontSize: '32px', marginRight: '10px' }} onClick={() => { setModalOpen(1); setModalBalance(lockinfo?.stakingTokenInfo.balance) }} disabled={pending}>
                          +
                        </StyledButton>
